...
/**
 * retail/master/merchregtype/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header merchregtype (mst_merchregtype)
 *
 ...
 * tanggal 13/07/2023
 */
$API = new class extends merchregtypeBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_merchregtype';
		$primarykey = 'merchregtype_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\merchregtype_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new merchregtype_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"merchregtype_id" => $obj->merchregtype_id
				];

				$criteriaValues = [
					"merchregtype_id" => " merchregtype_id = :merchregtype_id "
				];
				...
				$sqlFieldList = [
					'merchregtype_id' => 'A.`merchregtype_id`', 'merchregtype_name' => 'A.`merchregtype_name`', 'merchregtype_iscangenerate' => 'A.`merchregtype_iscangenerate`', 'merchregtype_descr' => 'A.`merchregtype_descr`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_merchregtype A";
				$sqlWhere = $where->sql;
