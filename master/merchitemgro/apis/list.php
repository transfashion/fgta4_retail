...
/**
 * retail/master/merchitemgro/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header merchitemgro (mst_merchitemgro)
...
$API = new class extends merchitemgroBase {
	...
	public function execute($options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\merchitemgro_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new merchitemgro_headerHandler($data, $options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.merchitemgro_id LIKE CONCAT('%', :search, '%') OR A.merchitemgro_name LIKE CONCAT('%', :search, '%') "
			];
			...
			$stmt = $this->db->prepare("select count(*) as n from mst_merchitemgro A" . $where->sql);
			...
			$stmt = $this->db->prepare("
				select 
				A.merchitemgro_id, A.merchitemgro_name, A.merchitemgro_nameshort, A.merchitemgro_descr, A.gender_id, A.brand_id, A._createby, A._createdate, A._modifyby, A._modifydate 
				from mst_merchitemgro A
