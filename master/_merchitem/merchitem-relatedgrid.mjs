...
const tbl_list = $('#pnl_editrelatedgrid-tbl_list');
const txt_title = $('#pnl_editrelatedgrid-title');
const pnl_control = $('.pnl_editrelatedgrid-control');
const btn_removechecked  = $('#pnl_editrelatedgrid-removechecked');
const btn_addrow = $('#pnl_editrelatedgrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.merchitem_name)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/related-list`
		options.criteria['id'] = data.merchitem_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editrelatedform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editrelatedform', () => {
		$ui.getPages().ITEMS['pnl_editrelatedform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/related-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editrelatedform', ()=>{
		$ui.getPages().ITEMS['pnl_editrelatedform'].handler.createnew(header_data)
	})	
}