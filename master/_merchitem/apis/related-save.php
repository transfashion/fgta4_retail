...
if (is_file(__DIR__ .'/data-related-handler.php')) {
	require_once __DIR__ .'/data-related-handler.php';
}
...
/**
 * retail/master/merchitem/apis/related-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel related merchitem (mst_merchitem)
 *
...
$API = new class extends merchitemBase {
	...
	public function execute($data, $options, $files) {
		$tablename = 'mst_merchitemrelated';
		$primarykey = 'merchitemrelated_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\merchitem_relatedHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new merchitem_relatedHandler($data, $options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				$sql = \FGTA4\utils\SqlUtility::Select($tablename , [
					$primarykey
					, 'merchitemrelated_id', 'merchitem_id', 'merchitem_art', 'merchitem_mat', 'merchitem_col', 'merchitem_size', 'brand_id', '_createby', '_createdate', '_modifyby', '_modifydate', '_createby', '_createdate', '_modifyby', '_modifydate'
				], $where->sql);
