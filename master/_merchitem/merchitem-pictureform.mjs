...
const fl_merchitempic_file_img = $('#pnl_editpictureform-fl_merchitempic_file_img');
const fl_merchitempic_file_lnk = $('#pnl_editpictureform-fl_merchitempic_file_link');				
			

const pnl_form = $('#pnl_editpictureform-form')
const obj = {
	txt_merchitempic_id: $('#pnl_editpictureform-txt_merchitempic_id'),
	txt_merchitempic_name: $('#pnl_editpictureform-txt_merchitempic_name'),
	txt_merchitempic_descr: $('#pnl_editpictureform-txt_merchitempic_descr'),
	txt_merchitempic_order: $('#pnl_editpictureform-txt_merchitempic_order'),
	fl_merchitempic_file: $('#pnl_editpictureform-fl_merchitempic_file'),
	txt_merchitem_id: $('#pnl_editpictureform-txt_merchitem_id')
}
...
export async function init(opt) {
	...
	form = new global.fgta4form(pnl_form, {
		primary: obj.txt_merchitempic_id,
		autoid: true,
		logview: 'mst_merchitempic',
		btn_edit: btn_edit,
		btn_save: btn_save,
	...
	obj.fl_merchitempic_file.filebox({
		onChange: function(value) {
			var files = obj.fl_merchitempic_file.filebox('files');
			var f = files[0];
				...
				return function(e) {
					if (loaded.type.startsWith('image')) {
						...
						image.onload = function() {
							fl_merchitempic_file_img.attr('src', e.target.result);
							fl_merchitempic_file_img.show();
							fl_merchitempic_file_lnk.hide();
						}
					} else {
						fl_merchitempic_file_img.hide();
						fl_merchitempic_file_lnk.hide();
					}
				}
			})(f);
...
export function createnew(hdata) {
	...
	form.createnew(async (data, options)=>{
		...
		fl_merchitempic_file_img.hide();
		fl_merchitempic_file_lnk.hide();	
		obj.fl_merchitempic_file.filebox('clear');		
...
function updatefilebox(record) {
	...
	obj.fl_merchitempic_file.filebox('clear');			
	if (record.merchitempic_file_doc!=undefined) {
		if (record.merchitempic_file_doc.type.startsWith('image')) {
			fl_merchitempic_file_lnk.hide();
			fl_merchitempic_file_img.show();
			fl_merchitempic_file_img.attr('src', record.merchitempic_file_doc.attachmentdata);
		} else {
			fl_merchitempic_file_img.hide();
			fl_merchitempic_file_lnk.show();
			fl_merchitempic_file_lnk[0].onclick = () => {
				fl_merchitempic_file_lnk.attr('download', record.merchitempic_file_doc.name);
				fl_merchitempic_file_lnk.attr('href', record.merchitempic_file_doc.attachmentdata);
			}
		}	
	} else {
		fl_merchitempic_file_img.hide();
...
function form_idsetup(options) {
	var objid = obj.txt_merchitempic_id
