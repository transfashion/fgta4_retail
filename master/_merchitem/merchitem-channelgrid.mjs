...
const tbl_list = $('#pnl_editchannelgrid-tbl_list');
const txt_title = $('#pnl_editchannelgrid-title');
const pnl_control = $('.pnl_editchannelgrid-control');
const btn_removechecked  = $('#pnl_editchannelgrid-removechecked');
const btn_addrow = $('#pnl_editchannelgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/channel-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editchannelform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editchannelform', () => {
		$ui.getPages().ITEMS['pnl_editchannelform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/channel-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editchannelform', ()=>{
		$ui.getPages().ITEMS['pnl_editchannelform'].handler.createnew(header_data)
	})	
}