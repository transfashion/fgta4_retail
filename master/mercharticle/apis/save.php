...
/**
 * retail/master/mercharticle/apis/save.php
 *
 * ====
 * Save
 * ====
...
$API = new class extends mercharticleBase {
	...
	public function execute($data, $options, $files) {
		...
		try {
			...
			if ($obj->mercharticle_matname=='') { $obj->mercharticle_matname = '--NULL--'; }
			if ($obj->mercharticle_name=='') { $obj->mercharticle_name = '--NULL--'; }
			if ($obj->mercharticle_couchdbid=='') { $obj->mercharticle_couchdbid = '--NULL--'; }
			if ($obj->itemstock_id=='') { $obj->itemstock_id = '--NULL--'; }
			if ($obj->mercharticle_updatebatch=='') { $obj->mercharticle_updatebatch = '--NULL--'; }
			...
			try {
				...
				$fieldname = 'mercharticle_picture';	
				if (property_exists($files, $fieldname)) {
					...
					$objfile->mercharticle_picture = $rev;
