...
/**
 * retail/master/mercharticle/apis/picture-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends mercharticleBase {
	...
	public function execute($data, $options, $files) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new mercharticle_pictureHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			if ($obj->merchpic_couchdbid=='') { $obj->merchpic_couchdbid = '--NULL--'; }
			if ($obj->merchpic_col=='') { $obj->merchpic_col = '--NULL--'; }
