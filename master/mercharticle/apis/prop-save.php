...
/**
 * retail/master/mercharticle/apis/prop-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel prop mercharticle (mst_mercharticleprop)
 *
...
$API = new class extends mercharticleBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'mst_mercharticleprop';
		$primarykey = 'mercharticleprop_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\mercharticle_propHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new mercharticle_propHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"mercharticleprop_id" => $obj->mercharticleprop_id
				];

				$criteriaValues = [
					"mercharticleprop_id" => " mercharticleprop_id = :mercharticleprop_id "
				];
				...
				$sqlFieldList = [
					'mercharticleprop_id' => 'A.`mercharticleprop_id`', 'itemproptype_id' => 'A.`itemproptype_id`', 'mercharticleprop_keys' => 'A.`mercharticleprop_keys`', 'mercharticleprop_value' => 'A.`mercharticleprop_value`',
					'mercharticle_id' => 'A.`mercharticle_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_mercharticleprop A";
				$sqlWhere = $where->sql;
