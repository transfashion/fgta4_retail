...
import * as hnd from  './mercharticle-variancegrid-hnd.mjs'

const tbl_list = $('#pnl_editvariancegrid-tbl_list');
const txt_title = $('#pnl_editvariancegrid-title');
const pnl_control = $('.pnl_editvariancegrid-control');
const btn_removechecked  = $('#pnl_editvariancegrid-removechecked');
const btn_addrow = $('#pnl_editvariancegrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.mercharticle_art)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/variance-list`
		options.criteria['id'] = data.mercharticle_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editvarianceform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editvarianceform', () => {
		$ui.getPages().ITEMS['pnl_editvarianceform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/variance-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editvarianceform', ()=>{
		$ui.getPages().ITEMS['pnl_editvarianceform'].handler.createnew(header_data)
	})	
}