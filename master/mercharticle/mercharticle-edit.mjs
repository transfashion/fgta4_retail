...
const fl_mercharticle_picture_img = $('#pnl_edit-fl_mercharticle_picture_img');
const fl_mercharticle_picture_lnk = $('#pnl_edit-fl_mercharticle_picture_link');				
...
const obj = {
	...
	txt_mercharticle_matname: $('#pnl_edit-txt_mercharticle_matname'),
	...
	txt_mercharticle_couchdbid: $('#pnl_edit-txt_mercharticle_couchdbid'),
	fl_mercharticle_picture: $('#pnl_edit-fl_mercharticle_picture'),
	txt_itemstock_id: $('#pnl_edit-txt_itemstock_id'),
	chk_mercharticle_isvarcolor: $('#pnl_edit-chk_mercharticle_isvarcolor'),
	chk_mercharticle_isvarsize: $('#pnl_edit-chk_mercharticle_isvarsize'),
	chk_mercharticle_isupdating: $('#pnl_edit-chk_mercharticle_isupdating'),
	txt_mercharticle_updatebatch: $('#pnl_edit-txt_mercharticle_updatebatch')
}
...
export async function init(opt) {
	...
	obj.fl_mercharticle_picture.filebox({
		onChange: function(value) {
			var files = obj.fl_mercharticle_picture.filebox('files');
			var f = files[0];
				...
				return function(e) {
					if (loaded.type.startsWith('image')) {
						...
						image.onload = function() {
							fl_mercharticle_picture_img.attr('src', e.target.result);
							fl_mercharticle_picture_img.show();
							fl_mercharticle_picture_lnk.hide();
						}
					} else {
						fl_mercharticle_picture_img.hide();
						fl_mercharticle_picture_lnk.hide();
					}
				}
			})(f);
...
export function createnew() {
	form.createnew(async (data, options)=>{
		...
		data.mercharticle_isvarcolor = '0'
		data.mercharticle_isvarsize = '0'
		data.mercharticle_isupdating = '0'
		...
		fl_mercharticle_picture_img.hide();
		fl_mercharticle_picture_lnk.hide();	
		obj.fl_mercharticle_picture.filebox('clear');		
		...
		$ui.getPages().ITEMS['pnl_editvariancegrid'].handler.createnew(data, options)
...
function updatefilebox(record) {
	// apabila ada keperluan untuk menampilkan data dari object storage
		...
		obj.fl_mercharticle_picture.filebox('clear');			
		if (record.mercharticle_picture_doc!=undefined) {
			if (record.mercharticle_picture_doc.type.startsWith('image')) {
				fl_mercharticle_picture_lnk.hide();
				fl_mercharticle_picture_img.show();
				fl_mercharticle_picture_img.attr('src', record.mercharticle_picture_doc.attachmentdata);
			} else {
				fl_mercharticle_picture_img.hide();
				fl_mercharticle_picture_lnk.show();
				fl_mercharticle_picture_lnk[0].onclick = () => {
					fl_mercharticle_picture_lnk.attr('download', record.mercharticle_picture_doc.name);
					fl_mercharticle_picture_lnk.attr('href', record.mercharticle_picture_doc.attachmentdata);
				}
			}	
		} else {
			fl_mercharticle_picture_img.hide();
