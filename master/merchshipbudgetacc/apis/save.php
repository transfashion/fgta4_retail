...
/**
 * retail/master/merchshipbudgetacc/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header merchshipbudgetacc (mst_merchshipbudgetacc)
 *
 ...
 * tanggal 13/10/2023
 */
$API = new class extends merchshipbudgetaccBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_merchshipbudgetacc';
		$primarykey = 'merchshipbudgetacc_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\merchshipbudgetacc_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new merchshipbudgetacc_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->merchshipbudgetacc_id = strtoupper($obj->merchshipbudgetacc_id);
			$obj->merchshipbudgetacc_name = strtoupper($obj->merchshipbudgetacc_name);
			$obj->merchshipbudgetacc_group = strtoupper($obj->merchshipbudgetacc_group);
			...
			try {
				...
				// result
				$options->criteria = [
					"merchshipbudgetacc_id" => $obj->merchshipbudgetacc_id
				];

				$criteriaValues = [
					"merchshipbudgetacc_id" => " merchshipbudgetacc_id = :merchshipbudgetacc_id "
				];
				...
				$sqlFieldList = [
					'merchshipbudgetacc_id' => 'A.`merchshipbudgetacc_id`', 'merchshipbudgetacc_name' => 'A.`merchshipbudgetacc_name`', 'merchshipbudgetacc_group' => 'A.`merchshipbudgetacc_group`', 'merchshipbudgetacc_isexclude' => 'A.`merchshipbudgetacc_isexclude`',
					'coa_id' => 'A.`coa_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_merchshipbudgetacc A";
				$sqlWhere = $where->sql;
