...
/**
 * retail/master/merchitemctg/apis/picture-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends merchitemctgBase {
	...
	public function execute($data, $options, $files) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new merchitemctg_pictureHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				$fieldname = 'merchitemctgpic_file';	
				if (property_exists($files, $fieldname)) {
					...
					$objfile->merchitemctgpic_file = $rev;
				...
				// result
				$options->criteria = [
					"merchitemctgpic_id" => $obj->merchitemctgpic_id
				];
