...
/**
 * retail/master/merchitemctg/apis/prop-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel prop merchitemctg (mst_merchitemctgprop)
 *
...
$API = new class extends merchitemctgBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'mst_merchitemctgprop';
		$primarykey = 'merchitemctgprop_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\merchitemctg_propHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new merchitemctg_propHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->merchproptype_id = strtoupper($obj->merchproptype_id);
			...
			try {
				...
				// result
				$options->criteria = [
					"merchitemctgprop_id" => $obj->merchitemctgprop_id
				];

				$criteriaValues = [
					"merchitemctgprop_id" => " merchitemctgprop_id = :merchitemctgprop_id "
				];
				...
				$sqlFieldList = [
					'merchitemctgprop_id' => 'A.`merchitemctgprop_id`', 'merchproptype_id' => 'A.`merchproptype_id`', 'merchitemctgprop_value' => 'A.`merchitemctgprop_value`', 'merchitemctg_id' => 'A.`merchitemctg_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_merchitemctgprop A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'merchproptype_name' => \FGTA4\utils\SqlUtility::Lookup($record['merchproptype_id'], $this->db, 'mst_merchproptype', 'merchproptype_id', 'merchproptype_name'),
