...
/**
 * retail/master/merchbilltype/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header merchbilltype (mst_merchbilltype)
 *
 ...
 * tanggal 31/07/2023
 */
$API = new class extends merchbilltypeBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_merchbilltype';
		$primarykey = 'merchbilltype_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\merchbilltype_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new merchbilltype_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->merchbilltype_id = strtoupper($obj->merchbilltype_id);
			$obj->merchbilltype_name = strtoupper($obj->merchbilltype_name);
			...
			try {
				...
				// result
				$options->criteria = [
					"merchbilltype_id" => $obj->merchbilltype_id
				];

				$criteriaValues = [
					"merchbilltype_id" => " merchbilltype_id = :merchbilltype_id "
				];
				...
				$sqlFieldList = [
					'merchbilltype_id' => 'A.`merchbilltype_id`', 'merchbilltype_name' => 'A.`merchbilltype_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_merchbilltype A";
				$sqlWhere = $where->sql;
