...
import * as hnd from  './promoabrule-sectiongrid-hnd.mjs'

const tbl_list = $('#pnl_editsectiongrid-tbl_list');
const txt_title = $('#pnl_editsectiongrid-title');
const pnl_control = $('.pnl_editsectiongrid-control');
const btn_removechecked  = $('#pnl_editsectiongrid-removechecked');
const btn_addrow = $('#pnl_editsectiongrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.promoabrule_name)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/section-list`
		options.criteria['id'] = data.promoabrule_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editsectionform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editsectiongrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editsectiongrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editsectionform', () => {
		$ui.getPages().ITEMS['pnl_editsectionform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/section-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editsectionform', ()=>{
		$ui.getPages().ITEMS['pnl_editsectionform'].handler.createnew(header_data)
	})	
}