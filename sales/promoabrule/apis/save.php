...
/**
 * retail/sales/promoabrule/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header promoabrule (mst_promoabrule)
 *
...
$API = new class extends promoabruleBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_promoabrule';
		$primarykey = 'promoabrule_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\promoabrule_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new promoabrule_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->promoabrule_id = strtoupper($obj->promoabrule_id);
			...
			try {
				...
				// result
				$options->criteria = [
					"promoabrule_id" => $obj->promoabrule_id
				];

				$criteriaValues = [
					"promoabrule_id" => " promoabrule_id = :promoabrule_id "
				];
				...
				$sqlFieldList = [
					'promoabrule_id' => 'A.`promoabrule_id`', 'promoablevel_id' => 'A.`promoablevel_id`', 'promoabrule_code' => 'A.`promoabrule_code`', 'promoabrule_name' => 'A.`promoabrule_name`',
					'promoabrule_descr' => 'A.`promoabrule_descr`', 'promoabrule_ishasgroupa' => 'A.`promoabrule_ishasgroupa`', 'promoabrule_ishasgroupb' => 'A.`promoabrule_ishasgroupb`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_promoabrule A";
				$sqlWhere = $where->sql;
