...
/**
 * retail/sales/promoabrule/apis/section-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends promoabruleBase {
	...
	public function execute($data, $options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new promoabrule_sectionHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'promoabrulesection_id', 'promoabrulesection_name', 'promoabrulesection_descr', 'promoabrule_id'
			];
			...
			try {
				...
				$header_table = 'mst_promoabrule';
				$header_primarykey = 'promoabrule_id';
				$detil_primarykey = 'promoabrule_id';
				...
				// result
				$options->criteria = [
					"promoabrulesection_id" => $obj->promoabrulesection_id
				];
