...
/**
 * retail/sales/promoab/apis/site-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel site promoab (mst_promoabsite)
 *
...
$API = new class extends promoabBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_promoabsite';
		$primarykey = 'promoabsite_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\promoab_siteHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new promoab_siteHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'promoabsite_id', 'site_id', 'site_code', 'promoab_id'
			];
			...
			$obj->promoabsite_id = strtoupper($obj->promoabsite_id);
			...
			try {
				...
				// result
				$options->criteria = [
					"promoabsite_id" => $obj->promoabsite_id
				];

				$criteriaValues = [
					"promoabsite_id" => " promoabsite_id = :promoabsite_id "
				];
				...
				$sqlFieldList = [
					'promoabsite_id' => 'A.`promoabsite_id`', 'site_id' => 'A.`site_id`', 'site_code' => 'A.`site_code`', 'promoab_id' => 'A.`promoab_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_promoabsite A";
				$sqlWhere = $where->sql;
