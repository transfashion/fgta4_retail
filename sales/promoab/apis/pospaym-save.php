...
/**
 * retail/sales/promoab/apis/pospaym-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel pospaym promoab (mst_promoabpospaym)
 *
...
$API = new class extends promoabBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_promoabpospaym';
		$primarykey = 'promoabpospaym_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\promoab_pospaymHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new promoab_pospaymHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->promoabpospaym_id = strtoupper($obj->promoabpospaym_id);
			...
			try {
				...
				// result
				$options->criteria = [
					"promoabpospaym_id" => $obj->promoabpospaym_id
				];

				$criteriaValues = [
					"promoabpospaym_id" => " promoabpospaym_id = :promoabpospaym_id "
				];
				...
				$sqlFieldList = [
					'promoabpospaym_id' => 'A.`promoabpospaym_id`', 'pospaym_code' => 'A.`pospaym_code`', 'promoab_id' => 'A.`promoab_id`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_promoabpospaym A";
				$sqlWhere = $where->sql;
