...
function match_basket(dgv, promo, fn_match) {
	...
	if (totalValueOri>=promo.data.valuethreshold && totalQty>=promo.data.qtythreshold) {
		for (var gr in ItemMatchs) {
			...
			var proDiscvalGr = promo.data.itemA.discval * (match.subtotalValueOri / totalValueOri);

			for (var matchrow of match.basketrow) {
				var rowdata = dgv.DATA[matchrow.id];
				var proDiscVal = proDiscvalGr * (rowdata.subtotal_ori / match.subtotalValueOri);
				var basketrow = {
					id: matchrow.id,
					rowdata: rowdata,
					promovalue: {
...
function doPromo_Simple(promo, dgv, basketrow) {
	...
	rowdata.subt_paymdiscval1 = basketrow.promovalue.discval;
	rowdata.item.B2_proprog_id = promo.id;
