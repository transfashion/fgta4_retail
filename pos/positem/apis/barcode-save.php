...
/**
 * retail/pos/positem/apis/barcode-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel barcode positem (pos_itemstockbarcode)
 *
...
$API = new class extends positemBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'pos_itemstockbarcode';
		$primarykey = 'positemstockbarcode_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\positem_barcodeHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new positem_barcodeHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				$header_table = 'pos_itemstock';
				$header_primarykey = 'positemstock_id';
				...
				// result
				$options->criteria = [
					"positemstockbarcode_id" => $obj->positemstockbarcode_id
				];

				$criteriaValues = [
					"positemstockbarcode_id" => " positemstockbarcode_id = :positemstockbarcode_id "
				];
				...
				$sqlFieldList = [
					'positemstockbarcode_id' => 'A.`positemstockbarcode_id`', 'site_id' => 'A.`site_id`', 'itemstock_id' => 'A.`itemstock_id`', 'itemstockbarcode_text' => 'A.`itemstockbarcode_text`',
					'dept_id' => 'A.`dept_id`', 'itemstockbarcode_isupdating' => 'A.`itemstockbarcode_isupdating`', 'itemstockbarcode_isinternal' => 'A.`itemstockbarcode_isinternal`', 'positemstock_id' => 'A.`positemstock_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "pos_itemstockbarcode A";
				$sqlWhere = $where->sql;
