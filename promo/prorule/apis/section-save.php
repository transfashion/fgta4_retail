...
/**
 * retail/promo/prorule/apis/section-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel section prorule (mst_prorulesec)
 *
...
$API = new class extends proruleBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_prorulesec';
		$primarykey = 'prorulesec_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\prorule_sectionHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new prorule_sectionHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				$header_table = 'mst_prorule';
				$header_primarykey = 'prorule_id';
				...
				// result
				$options->criteria = [
					"prorulesec_id" => $obj->prorulesec_id
				];

				$criteriaValues = [
					"prorulesec_id" => " prorulesec_id = :prorulesec_id "
				];
				...
				$sqlFieldList = [
					'prorulesec_id' => 'A.`prorulesec_id`', 'prorulesec_name' => 'A.`prorulesec_name`', 'prorulesec_label' => 'A.`prorulesec_label`', 'prorulesec_descr' => 'A.`prorulesec_descr`',
					'prorule_id' => 'A.`prorule_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_prorulesec A";
				$sqlWhere = $where->sql;
