...
/**
 * retail/promo/problock/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header problock (mst_problock)
 *
 ...
 * tanggal 01/04/2023
 */
$API = new class extends problockBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_problock';
		$primarykey = 'problock_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\problock_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new problock_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->problock_id = strtoupper($obj->problock_id);
			$obj->problock_name = strtoupper($obj->problock_name);
			...
			try {
				...
				// result
				$options->criteria = [
					"problock_id" => $obj->problock_id
				];

				$criteriaValues = [
					"problock_id" => " problock_id = :problock_id "
				];
				...
				$sqlFieldList = [
					'problock_id' => 'A.`problock_id`', 'problock_name' => 'A.`problock_name`', 'problock_descr' => 'A.`problock_descr`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_problock A";
				$sqlWhere = $where->sql;
