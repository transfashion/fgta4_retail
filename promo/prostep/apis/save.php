...
/**
 * retail/promo/prostep/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header prostep (mst_prostep)
 *
 ...
 * tanggal 28/03/2023
 */
$API = new class extends prostepBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_prostep';
		$primarykey = 'prostep_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\prostep_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new prostep_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->prostep_id = strtoupper($obj->prostep_id);
			$obj->prostep_name = strtoupper($obj->prostep_name);
			...
			try {
				...
				// result
				$options->criteria = [
					"prostep_id" => $obj->prostep_id
				];

				$criteriaValues = [
					"prostep_id" => " prostep_id = :prostep_id "
				];
				...
				$sqlFieldList = [
					'prostep_id' => 'A.`prostep_id`', 'prostep_name' => 'A.`prostep_name`', 'prostep_descr' => 'A.`prostep_descr`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_prostep A";
				$sqlWhere = $where->sql;
