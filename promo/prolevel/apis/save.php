...
/**
 * retail/promo/prolevel/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header prolevel (mst_prolevel)
 *
 ...
 * tanggal 28/03/2023
 */
$API = new class extends prolevelBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_prolevel';
		$primarykey = 'prolevel_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\prolevel_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new prolevel_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->prolevel_id = strtoupper($obj->prolevel_id);
			$obj->prolevel_name = strtoupper($obj->prolevel_name);
			...
			try {
				...
				// result
				$options->criteria = [
					"prolevel_id" => $obj->prolevel_id
				];

				$criteriaValues = [
					"prolevel_id" => " prolevel_id = :prolevel_id "
				];
				...
				$sqlFieldList = [
					'prolevel_id' => 'A.`prolevel_id`', 'prolevel_name' => 'A.`prolevel_name`', 'prolevel_descr' => 'A.`prolevel_descr`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_prolevel A";
				$sqlWhere = $where->sql;
