...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.proprog_id)
	header_data = data;
}
...
function grd_list_rowrender(tr) {
	...
	$(tr).find('td').each((i, td) => {
		...
		if (mapping=='docauth_descr') {
			...
			var authby = "";
			var notes = "";			
			if (record.proprogappr_isdeclined=="1") {
				gambar = `<img src="images/xtiondecl.png" width="20px" height="20px">`;
				authby = `<div><span style="font-weight: bold">${record.proprogappr_declinedby}</span> (${record.proprogappr_declineddate})</div>`
				notes = record.proprogappr_notes.replace(/(?:\r\n|\r|\n)/g, '<br>');
				notes = `<div style="margin-top: 5px; font-style: italic">${notes}</div>`
			} else if (record.proprogappr_isapproved=="1") {
				gambar = `<img src="images/xtionappr.png" width="20px" height="20px">`;
				authby = `<div><span style="font-weight: bold">${record.proprogappr_by}</span> (${record.proprogappr_date})</div>`
			} else {
