...
/**
 * retail/promo/proprog/apis/site-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel site proprog (mst_prosite)
 *
...
$API = new class extends proprogBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_prosite';
		$primarykey = 'prosite_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\proprog_siteHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new proprog_siteHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"prosite_id" => $obj->prosite_id
				];

				$criteriaValues = [
					"prosite_id" => " prosite_id = :prosite_id "
				];
				...
				$sqlFieldList = [
					'prosite_id' => 'A.`prosite_id`', 'site_id' => 'A.`site_id`', 'proprog_id' => 'A.`proprog_id`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_prosite A";
				$sqlWhere = $where->sql;
