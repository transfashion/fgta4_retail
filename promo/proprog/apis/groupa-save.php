...
if (is_file(__DIR__ .'/data-groupa-handler.php')) {
	require_once __DIR__ .'/data-groupa-handler.php';
}
...
/**
 * retail/promo/proprog/apis/groupa-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel groupa proprog (mst_proprogitemstocka)
 *
...
$API = new class extends proprogBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_proprogitemstocka';
		$primarykey = 'proprogitemstocka_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\proprog_groupaHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new proprog_groupaHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			if ($obj->itemstock_tag=='') { $obj->itemstock_tag = '--NULL--'; }
			...
			try {
				...
				$header_table = 'mst_proprog';
				$header_primarykey = 'proprog_id';
				...
				// result
				$options->criteria = [
					"proprogitemstocka_id" => $obj->proprogitemstocka_id
				];

				$criteriaValues = [
					"proprogitemstocka_id" => " proprogitemstocka_id = :proprogitemstocka_id "
				];
				...
				$sqlFieldList = [
					'proprogitemstocka_id' => 'A.`proprogitemstocka_id`', 'itemstock_id' => 'A.`itemstock_id`', 'itemstock_tag' => 'A.`itemstock_tag`', 'itemstock_sellprice' => 'A.`itemstock_sellprice`',
					'itemstock_discval' => 'A.`itemstock_discval`', 'itemstock_disc' => 'A.`itemstock_disc`', 'proprog_id' => 'A.`proprog_id`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_proprogitemstocka A";
				$sqlWhere = $where->sql;
