...
if (is_file(__DIR__ .'/data-pospaym-handler.php')) {
	require_once __DIR__ .'/data-pospaym-handler.php';
}
...
/**
 * retail/promo/proprog/apis/pospaym-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel pospaym proprog (mst_propaymethod)
 *
...
$API = new class extends proprogBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_propaymethod';
		$primarykey = 'propaymethod_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\proprog_pospaymHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new proprog_pospaymHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"propaymethod_id" => $obj->propaymethod_id
				];

				$criteriaValues = [
					"propaymethod_id" => " propaymethod_id = :propaymethod_id "
				];
				...
				$sqlFieldList = [
					'propaymethod_id' => 'A.`propaymethod_id`', 'paymethod_id' => 'A.`paymethod_id`', 'paymethod_code' => 'A.`paymethod_code`', 'proprog_id' => 'A.`proprog_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_propaymethod A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'paymethod_name' => \FGTA4\utils\SqlUtility::Lookup($record['paymethod_id'], $this->db, 'pos_paymethod', 'paymethod_id', 'paymethod_name'),
