...
/**
 * retail/promo/proprog/apis/pospaym-list.php
 *
 * ==============
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel pospaym proprog (mst_propaymethod)
...
$API = new class extends proprogBase {
	...
	public function execute($options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new proprog_pospaymHandler($options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('paymethod_name', 'paymethod_id', $record, 'pos_paymethod', 'paymethod_name', 'paymethod_id');
