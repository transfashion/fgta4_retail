...
if (is_file(__DIR__ .'/data-groupb-handler.php')) {
	require_once __DIR__ .'/data-groupb-handler.php';
}
...
/**
 * retail/promo/proprog/apis/groupb-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel groupb proprog (mst_proprogitemstockb)
 *
...
$API = new class extends proprogBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_proprogitemstockb';
		$primarykey = 'proprogitemstockb_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\proprog_groupbHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new proprog_groupbHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"proprogitemstockb_id" => $obj->proprogitemstockb_id
				];

				$criteriaValues = [
					"proprogitemstockb_id" => " proprogitemstockb_id = :proprogitemstockb_id "
				];
				...
				$sqlFieldList = [
					'proprogitemstockb_id' => 'A.`proprogitemstockb_id`', 'itemstock_id' => 'A.`itemstock_id`', 'itemstock_sellprice' => 'A.`itemstock_sellprice`', 'itemstock_discval' => 'A.`itemstock_discval`',
					'itemstock_disc' => 'A.`itemstock_disc`', 'proprog_id' => 'A.`proprog_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_proprogitemstockb A";
				$sqlWhere = $where->sql;
