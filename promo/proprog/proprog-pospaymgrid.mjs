...
import * as hnd from  './proprog-pospaymgrid-hnd.mjs'

const tbl_list = $('#pnl_editpospaymgrid-tbl_list');
const txt_title = $('#pnl_editpospaymgrid-title');
const pnl_control = $('.pnl_editpospaymgrid-control');
const btn_removechecked  = $('#pnl_editpospaymgrid-removechecked');
const btn_addrow = $('#pnl_editpospaymgrid-addrow');
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpospaymform', () => {
		$ui.getPages().ITEMS['pnl_editpospaymform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/pospaym-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpospaymform', ()=>{
		$ui.getPages().ITEMS['pnl_editpospaymform'].handler.createnew(header_data)
	})	
}