...
import * as hnd from  './proprog-groupbgrid-hnd.mjs'

const tbl_list = $('#pnl_editgroupbgrid-tbl_list');
const txt_title = $('#pnl_editgroupbgrid-title');
const pnl_control = $('.pnl_editgroupbgrid-control');
const btn_removechecked  = $('#pnl_editgroupbgrid-removechecked');
const btn_addrow = $('#pnl_editgroupbgrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.proprog_display)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/groupb-list`
		options.criteria['id'] = data.proprog_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editgroupbform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editgroupbform', () => {
		$ui.getPages().ITEMS['pnl_editgroupbform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/groupb-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editgroupbform', ()=>{
		$ui.getPages().ITEMS['pnl_editgroupbform'].handler.createnew(header_data)
	})	
}