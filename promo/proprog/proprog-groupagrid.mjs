...
import * as hnd from  './proprog-groupagrid-hnd.mjs'

const tbl_list = $('#pnl_editgroupagrid-tbl_list');
const txt_title = $('#pnl_editgroupagrid-title');
const pnl_control = $('.pnl_editgroupagrid-control');
const btn_removechecked  = $('#pnl_editgroupagrid-removechecked');
const btn_addrow = $('#pnl_editgroupagrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/groupa-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editgroupaform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editgroupaform', () => {
		$ui.getPages().ITEMS['pnl_editgroupaform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/groupa-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editgroupaform', ()=>{
		$ui.getPages().ITEMS['pnl_editgroupaform'].handler.createnew(header_data)
	})	
}