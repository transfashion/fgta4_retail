...
/**
 * retail/promo/prospot/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header prospot (mst_prospot)
 *
 ...
 * tanggal 28/03/2023
 */
$API = new class extends prospotBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_prospot';
		$primarykey = 'prospot_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\prospot_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new prospot_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->prospot_id = strtoupper($obj->prospot_id);
			$obj->prospot_name = strtoupper($obj->prospot_name);
			...
			try {
				...
				// result
				$options->criteria = [
					"prospot_id" => $obj->prospot_id
				];

				$criteriaValues = [
					"prospot_id" => " prospot_id = :prospot_id "
				];
				...
				$sqlFieldList = [
					'prospot_id' => 'A.`prospot_id`', 'prospot_name' => 'A.`prospot_name`', 'prospot_descr' => 'A.`prospot_descr`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_prospot A";
				$sqlWhere = $where->sql;
