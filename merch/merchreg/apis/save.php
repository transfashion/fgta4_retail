...
/**
 * retail/merch/merchreg/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header merchreg (trn_merchreg)
 *
...
$API = new class extends merchregBase {
	...
	public function execute($data, $options) {
		...
		try {
			...
			$obj->merchreg_date = (\DateTime::createFromFormat('d/m/Y',$obj->merchreg_date))->format('Y-m-d');



			if ($obj->merchreg_descr=='') { $obj->merchreg_descr = '--NULL--'; }
			if ($obj->interface_id=='') { $obj->interface_id = '--NULL--'; }


			unset($obj->merchreg_iscommit);
			unset($obj->merchreg_commitby);
			unset($obj->merchreg_commitdate);
			unset($obj->merchreg_isgenerate);
			unset($obj->merchreg_generateby);
			unset($obj->merchreg_generatedate);
			...
			try {
				...
				$sqlFieldList = [
					...
					'curr_rate' => 'A.`curr_rate`', 'partner_id' => 'A.`partner_id`', 'merchsea_id' => 'A.`merchsea_id`', 'dept_id' => 'A.`dept_id`',
					'unit_id' => 'A.`unit_id`', 'brand_id' => 'A.`brand_id`', 'merchregtype_id' => 'A.`merchregtype_id`', 'interface_id' => 'A.`interface_id`',
					'merchregtype_iscangenerate' => 'A.`merchregtype_iscangenerate`', 'merchreg_version' => 'A.`merchreg_version`', 'merchreg_iscommit' => 'A.`merchreg_iscommit`', 'merchreg_commitby' => 'A.`merchreg_commitby`',
				...
				$dataresponse = array_merge($record, [
					...
					'merchreg_date' => date("d/m/Y", strtotime($row['merchreg_date'])),
