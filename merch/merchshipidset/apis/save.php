...
/**
 * retail/merch/merchshipidset/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header merchshipidset (trn_merchshipidset)
 *
 ...
 * tanggal 06/11/2023
 */
$API = new class extends merchshipidsetBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'trn_merchshipidset';
		$primarykey = 'merchshipidset_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\merchshipidset_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new merchshipidset_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"merchshipidset_id" => $obj->merchshipidset_id
				];

				$criteriaValues = [
					"merchshipidset_id" => " merchshipidset_id = :merchshipidset_id "
				];
				...
				$sqlFieldList = [
					'merchshipidset_id' => 'A.`merchshipidset_id`', 'merchship_id' => 'A.`merchship_id`', 'merchshipidset_doc' => 'A.`merchshipidset_doc`', 'merchshipidset_value' => 'A.`merchshipidset_value`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "trn_merchshipidset A";
				$sqlWhere = $where->sql;
