...
/**
 * retail/merch/merchrv/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header merchrv (trn_merchrv)
...
$API = new class extends merchrvBase {
	...
	public function execute($options) {
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.merchrv_id LIKE CONCAT('%', :search, '%') OR A.merchrv_descr LIKE CONCAT('%', :search, '%') "
			];
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('merchorderout_descr', 'merchorderout_id', $record, 'trn_merchorderout', 'merchorderout_descr', 'merchorderout_id');
				...
				$this->addFields('merchrv_commitby', 'merchrv_commitby', $record, $GLOBALS['MAIN_USERTABLE'], 'user_fullname', 'user_id');
				$this->addFields('merchrv_postby', 'merchrv_postby', $record, $GLOBALS['MAIN_USERTABLE'], 'user_fullname', 'user_id');
