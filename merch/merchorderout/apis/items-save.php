...
/**
 * retail/merch/merchorderout/apis/items-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends merchorderoutBase {
	...
	public function execute($data, $options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new merchorderout_itemsHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'merchorderoutitem_id', 'merchitem_id', 'merchitem_combo', 'orderoutitem_descr',
					'orderoutitem_value', 'orderoutitem_qty', 'orderoutitem_subtotalvaluefrg', 'curr_id',
					'curr_rate', 'orderoutitem_subtotalvalueidr', 'orderoutitem_outstdqty', 'orderoutitem_outstdvaluefrg',
					'orderoutitem_outstdvalueidr', 'merchorderout_id'
			];
			...
			unset($obj->orderoutitem_outstdqty);
			unset($obj->orderoutitem_outstdvaluefrg);
			unset($obj->orderoutitem_outstdvalueidr);
			...
			try {
				...
				$header_table = 'trn_merchorderout';
				$header_primarykey = 'merchorderout_id';
				$detil_primarykey = 'merchorderout_id';
				...
				// result
				$options->criteria = [
					"merchorderoutitem_id" => $obj->merchorderoutitem_id
				];
