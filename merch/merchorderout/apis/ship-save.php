...
if (is_file(__DIR__ .'/data-ship-handler.php')) {
	require_once __DIR__ .'/data-ship-handler.php';
}
...
/**
 * retail/merch/merchorderout/apis/ship-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel ship merchorderout (trn_merchship)
 *
...
$API = new class extends merchorderoutBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\merchorderout_shipHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new merchorderout_shipHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'merchship_id', 'merchship_date', 'merchship_descr', 'merchorderout_id'
			];
			...
			try {
				...
				$sqlFieldList = [
					'merchship_id' => 'A.`merchship_id`', 'merchship_date' => 'A.`merchship_date`', 'merchship_descr' => 'A.`merchship_descr`', 'merchorderout_id' => 'A.`merchorderout_id`',
