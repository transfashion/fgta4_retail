...
import * as hnd from  './merchorderout-shipgrid-hnd.mjs'

const tbl_list = $('#pnl_editshipgrid-tbl_list');
const txt_title = $('#pnl_editshipgrid-title');
const pnl_control = $('.pnl_editshipgrid-control');
const btn_removechecked  = $('#pnl_editshipgrid-removechecked');
const btn_addrow = $('#pnl_editshipgrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.merchorderout_descr)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/ship-list`
		options.criteria['id'] = data.merchorderout_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editshipform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editshipgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editshipgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editshipform', () => {
		$ui.getPages().ITEMS['pnl_editshipform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/ship-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editshipform', ()=>{
		$ui.getPages().ITEMS['pnl_editshipform'].handler.createnew(header_data)
	})	
}