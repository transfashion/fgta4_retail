...
import * as hnd from  './merchorderout-shipform-hnd.mjs'

const reload_header_modified = true;

const txt_caption = $('#pnl_editshipform-caption')
const txt_title = $('#pnl_editshipform-title')
const btn_edit = $('#pnl_editshipform-btn_edit')
const btn_save = $('#pnl_editshipform-btn_save')
const btn_delete = $('#pnl_editshipform-btn_delete')
const btn_prev = $('#pnl_editshipform-btn_prev')
const btn_next = $('#pnl_editshipform-btn_next')
const btn_addnew = $('#pnl_editshipform-btn_addnew')
const chk_autoadd = $('#pnl_editshipform-autoadd')


const pnl_form = $('#pnl_editshipform-form')
const obj = {
	txt_merchship_id: $('#pnl_editshipform-txt_merchship_id'),
	dt_merchship_date: $('#pnl_editshipform-dt_merchship_date'),
	txt_merchship_descr: $('#pnl_editshipform-txt_merchship_descr'),
	txt_merchorderout_id: $('#pnl_editshipform-txt_merchorderout_id')
}
	...
	document.addEventListener('OnButtonBack', (ev) => {
		...
		if ($ui.getPages().getCurrentPage()==this_page_id) {
			...
			if (form.isDataChanged()) {
				form.canceledit(()=>{
					$ui.getPages().show('pnl_editshipgrid', ()=>{
						form.setViewMode()
						$ui.getPages().ITEMS['pnl_editshipgrid'].handler.scrolllast()
					})					
				})
			} else {
				$ui.getPages().show('pnl_editshipgrid', ()=>{
...
export function open(data, rowid, hdata) {
	...
	var fn_dataopening = async (options) => {
		options.api = `${global.modulefullname}/ship-open`
...
async function form_datasaving(data, options) {
	options.api = `${global.modulefullname}/ship-save`
...
async function form_datasaved(result, options) {
	...
	*/
	...
	form.rowid = $ui.getPages().ITEMS['pnl_editshipgrid'].handler.updategrid(data, form.rowid)
	var rowdata = {
		data: data,
		rowid: form.rowid
	}
...
async function form_deleting(data, options) {
	options.api = `${global.modulefullname}/ship-delete`
...
async function form_deleted(result, options) {
	...
	$ui.getPages().show('pnl_editshipgrid', ()=>{
		$ui.getPages().ITEMS['pnl_editshipgrid'].handler.removerow(form.rowid)
	});
...
function btn_prev_click() {
	...
	var record = $ui.getPages().ITEMS['pnl_editshipgrid'].handler.getGrid().DATA[dataid]
