...
import * as hnd from  './merchship-realisasigrid-hnd.mjs'

const tbl_list = $('#pnl_editrealisasigrid-tbl_list');
const txt_title = $('#pnl_editrealisasigrid-title');
const pnl_control = $('.pnl_editrealisasigrid-control');
const btn_removechecked  = $('#pnl_editrealisasigrid-removechecked');
const btn_addrow = $('#pnl_editrealisasigrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.merchship_descr)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/realisasi-list`
		options.criteria['id'] = data.merchship_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editrealisasiform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editrealisasigrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editrealisasigrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editrealisasiform', () => {
		$ui.getPages().ITEMS['pnl_editrealisasiform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/realisasi-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editrealisasiform', ()=>{
		$ui.getPages().ITEMS['pnl_editrealisasiform'].handler.createnew(header_data)
	})	
}