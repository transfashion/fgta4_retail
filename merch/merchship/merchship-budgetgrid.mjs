...
import * as hnd from  './merchship-budgetgrid-hnd.mjs'

const tbl_list = $('#pnl_editbudgetgrid-tbl_list');
const txt_title = $('#pnl_editbudgetgrid-title');
const pnl_control = $('.pnl_editbudgetgrid-control');
const btn_removechecked  = $('#pnl_editbudgetgrid-removechecked');
const btn_addrow = $('#pnl_editbudgetgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/budget-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editbudgetform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editbudgetgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editbudgetgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editbudgetform', () => {
		$ui.getPages().ITEMS['pnl_editbudgetform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/budget-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editbudgetform', ()=>{
		$ui.getPages().ITEMS['pnl_editbudgetform'].handler.createnew(header_data)
	})	
}