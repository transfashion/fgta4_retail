...
if (is_file(__DIR__ .'/data-realisasi-handler.php')) {
	require_once __DIR__ .'/data-realisasi-handler.php';
}
...
/**
 * retail/merch/merchship/apis/realisasi-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel realisasi merchship (trn_merchbillin)
 *
...
$API = new class extends merchshipBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\merchship_realisasiHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new merchship_realisasiHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'merchbillin_id', 'merchbillin_date', 'merchbillin_descr', 'merchship_id'
			];
			...
			$obj->merchbillin_date = (\DateTime::createFromFormat('d/m/Y',$obj->merchbillin_date))->format('Y-m-d');



			if ($obj->merchbillin_descr=='') { $obj->merchbillin_descr = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"merchbillin_id" => $obj->merchbillin_id
				];
				...
				$sqlFieldList = [
					'merchbillin_id' => 'A.`merchbillin_id`', 'merchbillin_date' => 'A.`merchbillin_date`', 'merchbillin_descr' => 'A.`merchbillin_descr`', 'merchship_id' => 'A.`merchship_id`',
				...
				$dataresponse = array_merge($record, [
					...
					'merchbillin_date' => date("d/m/Y", strtotime($row['merchbillin_date'])),
