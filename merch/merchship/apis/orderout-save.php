...
/**
 * retail/merch/merchship/apis/orderout-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends merchshipBase {
	...
	public function execute($data, $options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new merchship_orderoutHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'merchshiporderout_id', 'merchorderout_id', 'merchsea_id', 'curr_id',
					'orderout_qty', 'orderout_valuefrg', 'orderout_valueidr', 'merchship_id'
			];
			...
			if ($obj->merchshiporderout_id=='') { $obj->merchshiporderout_id = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"merchshiporderout_id" => $obj->merchshiporderout_id
				];
