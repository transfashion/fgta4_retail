...
if (is_file(__DIR__ .'/data-recv-handler.php')) {
	require_once __DIR__ .'/data-recv-handler.php';
}
...
/**
 * retail/merch/merchship/apis/recv-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel recv merchship (trn_merchrv)
 *
...
$API = new class extends merchshipBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\merchship_recvHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new merchship_recvHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'merchrv_id', 'merchrv_date', 'merchrv_descr', 'merchship_id'
			];
			...
			$obj->merchrv_date = (\DateTime::createFromFormat('d/m/Y',$obj->merchrv_date))->format('Y-m-d');



			if ($obj->merchrv_descr=='') { $obj->merchrv_descr = '--NULL--'; }
			...
			try {
				...
				$sqlFieldList = [
					'merchrv_id' => 'A.`merchrv_id`', 'merchrv_date' => 'A.`merchrv_date`', 'merchrv_descr' => 'A.`merchrv_descr`', 'merchship_id' => 'A.`merchship_id`',
				...
				$dataresponse = array_merge($record, [
					...
					'merchrv_date' => date("d/m/Y", strtotime($row['merchrv_date'])),
