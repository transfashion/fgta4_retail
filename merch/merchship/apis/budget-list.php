...
/**
 * retail/merch/merchship/apis/budget-list.php
 *
 * ==============
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel budget merchship (trn_merchshipbudget)
...
$API = new class extends merchshipBase {
	...
	public function execute($options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new merchship_budgetHandler($options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('merchshipbudgetacc_name', 'merchshipbudgetacc_id', $record, 'mst_merchshipbudgetacc', 'merchshipbudgetacc_name', 'merchshipbudgetacc_id');
