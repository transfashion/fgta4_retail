...
/**
 * retail/merch/merchship/apis/xtion-commit.php
 *
 * =======
 * Commit
 * =======
...
$API = new class extends merchshipBase {
	...
	public function execute($id, $options) {
		...
		try {
			...
			if (method_exists(get_class($hnd), 'XtionActionExecuting')) {
				...
				$hnd->XtionActionExecuting($id, 'verify', $currentdata);
			}
			...
			try {
				...
				if (method_exists(get_class($hnd), 'XtionVerify')) {
					// XtionVerify(string $id, object &$currentdata) : void
					$hnd->XtionVerify($id, $currentdata);
				}

	
				$this->save_and_set_verify_flag($id, $currentdata);
				if (method_exists(get_class($hnd), 'XtionVerified')) {
					// XtionVerified(string $id) : void
					$hnd->XtionVerified($id);
				}
				...
				$dataresponse = array_merge($record, [
					...
					'merchship_activateby' => \FGTA4\utils\SqlUtility::Lookup($record['merchship_activateby'], $this->db, $GLOBALS['MAIN_USERTABLE'], 'user_id', 'user_fullname'),
	...
	public function save_and_set_verify_flag($id, $currentdata) {
		try {
			$sql = " 
				update $this->main_tablename
				set
				merchship_isverify = 1,
				merchship_verifyby = :username, 
				merchship_verifydate  = :date
				where
