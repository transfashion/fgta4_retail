...
import * as hnd from  './merchship-orderoutgrid-hnd.mjs'

const tbl_list = $('#pnl_editorderoutgrid-tbl_list');
const txt_title = $('#pnl_editorderoutgrid-title');
const pnl_control = $('.pnl_editorderoutgrid-control');
const btn_removechecked  = $('#pnl_editorderoutgrid-removechecked');
const btn_addrow = $('#pnl_editorderoutgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/orderout-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editorderoutform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editorderoutgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editorderoutgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editorderoutform', () => {
		$ui.getPages().ITEMS['pnl_editorderoutform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/orderout-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editorderoutform', ()=>{
		$ui.getPages().ITEMS['pnl_editorderoutform'].handler.createnew(header_data)
	})	
}