...
import * as hnd from  './merchship-realisasiform-hnd.mjs'

const reload_header_modified = true;

const txt_caption = $('#pnl_editrealisasiform-caption')
const txt_title = $('#pnl_editrealisasiform-title')
const btn_edit = $('#pnl_editrealisasiform-btn_edit')
const btn_save = $('#pnl_editrealisasiform-btn_save')
const btn_delete = $('#pnl_editrealisasiform-btn_delete')
const btn_prev = $('#pnl_editrealisasiform-btn_prev')
const btn_next = $('#pnl_editrealisasiform-btn_next')
const btn_addnew = $('#pnl_editrealisasiform-btn_addnew')
const chk_autoadd = $('#pnl_editrealisasiform-autoadd')


const pnl_form = $('#pnl_editrealisasiform-form')
const obj = {
	txt_merchbillin_id: $('#pnl_editrealisasiform-txt_merchbillin_id'),
	dt_merchbillin_date: $('#pnl_editrealisasiform-dt_merchbillin_date'),
	txt_merchbillin_descr: $('#pnl_editrealisasiform-txt_merchbillin_descr'),
	txt_merchship_id: $('#pnl_editrealisasiform-txt_merchship_id')
}
	...
	document.addEventListener('OnButtonBack', (ev) => {
		...
		if ($ui.getPages().getCurrentPage()==this_page_id) {
			...
			if (form.isDataChanged()) {
				form.canceledit(()=>{
					$ui.getPages().show('pnl_editrealisasigrid', ()=>{
						form.setViewMode()
						$ui.getPages().ITEMS['pnl_editrealisasigrid'].handler.scrolllast()
					})					
				})
			} else {
				$ui.getPages().show('pnl_editrealisasigrid', ()=>{
...
export function open(data, rowid, hdata) {
	...
	var fn_dataopening = async (options) => {
		options.api = `${global.modulefullname}/realisasi-open`
...
async function form_datasaving(data, options) {
	options.api = `${global.modulefullname}/realisasi-save`
...
async function form_datasaved(result, options) {
	...
	*/
	...
	form.rowid = $ui.getPages().ITEMS['pnl_editrealisasigrid'].handler.updategrid(data, form.rowid)
	var rowdata = {
		data: data,
		rowid: form.rowid
	}
...
async function form_deleting(data, options) {
	options.api = `${global.modulefullname}/realisasi-delete`
...
async function form_deleted(result, options) {
	...
	$ui.getPages().show('pnl_editrealisasigrid', ()=>{
		$ui.getPages().ITEMS['pnl_editrealisasigrid'].handler.removerow(form.rowid)
	});
...
function btn_prev_click() {
	...
	var record = $ui.getPages().ITEMS['pnl_editrealisasigrid'].handler.getGrid().DATA[dataid]
