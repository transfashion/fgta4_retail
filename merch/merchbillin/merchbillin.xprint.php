...
class PrintForm extends WebModule {
	function __construct() {
		// $logfilepath = __LOCALDB_DIR . "/output/logfile-merchbillin.txt";
	...
	public function LoadPage() {
		...
		try {
			...
			$stmt->execute([':merchbillin_id' => $id]);
			$rows  = $stmt->fetchall(\PDO::FETCH_ASSOC);
			$row = $rows[0];

			$this->merchbillin_id = $row['merchbillin_id'];
	...
	function getSqlHeader() {
		return "
			...
			from trn_merchbillin A where merchbillin_id = :merchbillin_id
		";
	}	
