...
/**
 * retail/fashion/merchsync/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header merchsync (fsn_merchsync)
 *
...
$API = new class extends merchsyncBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'fsn_merchsync';
		$primarykey = 'merchsync_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\merchsync_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new merchsync_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->merchsync_id = strtoupper($obj->merchsync_id);
			...
			try {
				...
				// result
				$options->criteria = [
					"merchsync_id" => $obj->merchsync_id
				];

				$criteriaValues = [
					"merchsync_id" => " merchsync_id = :merchsync_id "
				];
