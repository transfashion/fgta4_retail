...
/**
 * retail/fashion/mercharticle/apis/item-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel item mercharticle (fsn_merchitem)
 *
...
$API = new class extends mercharticleBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\mercharticle_itemHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new mercharticle_itemHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'merchitem_id', 'merchitem_size', 'merchitem_isdisabled', 'unit_id',
					'brand_id', 'dept_id', 'mercharticle_id'
			];
			...
			try {
				...
				$header_table = 'fsn_mercharticle';
				$header_primarykey = 'mercharticle_id';
				$detil_primarykey = 'mercharticle_id';
				...
				$sqlFieldList = [
					'merchitem_id' => 'A.`merchitem_id`', 'merchitem_size' => 'A.`merchitem_size`', 'merchitem_isdisabled' => 'A.`merchitem_isdisabled`', 'unit_id' => 'A.`unit_id`',
					'brand_id' => 'A.`brand_id`', 'dept_id' => 'A.`dept_id`', 'mercharticle_id' => 'A.`mercharticle_id`', '_createby' => 'A.`_createby`',
