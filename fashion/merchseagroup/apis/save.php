...
/**
 * retail/fashion/merchseagroup/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header merchseagroup (fsn_merchseagroup)
 *
 ...
 * tanggal 02/09/2024
 */
$API = new class extends merchseagroupBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'fsn_merchseagroup';
		$primarykey = 'merchseagroup_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\merchseagroup_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new merchseagroup_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->merchseagroup_id = strtoupper($obj->merchseagroup_id);
			$obj->merchseagroup_name = strtoupper($obj->merchseagroup_name);
			...
			try {
				...
				// result
				$options->criteria = [
					"merchseagroup_id" => $obj->merchseagroup_id
				];

				$criteriaValues = [
					"merchseagroup_id" => " merchseagroup_id = :merchseagroup_id "
				];
				...
				$sqlFieldList = [
					'merchseagroup_id' => 'A.`merchseagroup_id`', 'merchseagroup_name' => 'A.`merchseagroup_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "fsn_merchseagroup A";
				$sqlWhere = $where->sql;
